<?php
    ...
    $review = "SELECT ulasanbuku.*, user.*, buku.* FROM ulasanbuku
                INNER JOIN user ON ulasanbuku.UserID = user.UserID
                INNER JOIN buku ON ulasanbuku.BukuID = buku.BukuID
                WHERE ulasanbuku.UserID = '$user_id'";

    $my_review = mysqli_query($conn, $review);
        ...
        <main class="w-4/5 ml-3">
            <div class="bg-white h-full p-4 rounded-lg shadow-md">
                <div class="bg-slate-200 p-6 border-sky-600 border-2 rounded-lg">
                    <p class="text-sky-600 text-xl font-semibold">Riwayat Ulasan</p>
                ...
                <div class="mt-4 grid grid-rows gap-4">
                    <?php foreach($my_review as $row): ?>
                    <div class="p-4 border-sky-600 border w-full rounded-lg">
                        <div class="flex justify-between items-center">
                            <p class="font-semibold"><?= $row['Judul']; ?></p>
                            <p><?= $row['Penulis']; ?></p>
                        </div>
                        <p class="my-1.5"><?= $row['Ulasan']; ?></p>
                        <div class="flex justify-between items-center">
                            <p class="text-slate-400 text-sm "><?= $row['Dikirim']; ?></p>
                            <?php if($row['Rating'] == 1): ?>
                            <p class="text-red-400">Kurang Puas</p>
                            <?php elseif($row['Rating'] == 2): ?>
                            <p class="text-yellow-400">Puas</p>
                            <?php elseif($row['Rating'] == 3): ?>
                            <p class="text-green-400">Sangat Puas</p>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endforeach; ?>
