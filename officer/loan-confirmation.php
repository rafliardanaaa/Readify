<?php
    ...
    $category_book = mysqli_query($conn, "SELECT kategoribuku_relasi.*, buku.*, kategoribuku.* FROM kategoribuku_relasi
                    INNER JOIN buku ON kategoribuku_relasi.BukuID = buku.BukuID
                    INNER JOIN kategribuku ON kategoribuku_relasi.KategoriID = kategoribuku.KategoriID
                    WHERE buku.BukuID = '$loan_id'");
    ...
    if(isset($_POST['confirm'])) {
        ...
        $result = loanConfirm($data);

        if($result) {
            echo "
                <script>
                    alert('Konfirmasi Buku Berhasil');
                    document.location.href = 'in-loan.php';
                </script>
            ";
        } else {
            echo "
                <script>
                    alert('Konfirmasi Buku gagal');
    ...
    <div class="flex">
        ...
        <main class="h-screen w-screen">
            ...
            <div class="px-8">
                <div class="flex justify-between">
                    <div class="px-2">
                        <ul class="flex items-center">
                            ...
                            <li class="inline-flex items-center">
                                <a href="loan-list.php" class="text-gray-400 hover:text-sky-600">Menunggu Respon</a>
                                <svg class="w-5 h-auto fill-current mx-2 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6-6-6z"/></svg>
                            </li>
                            <li class="inline-flex items-center">
                                <a href="" class="text-sky-600">Konfirmasi Pinjaman</a>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="flex">
                    <?php foreach($loan_list as $row): ?>
                    <div class="bg-white h-[720px] w-4/5 mr-5 p-4 rounded-lg shadow-md relative">
                        <div class="bg-sky-600 px-4 py-2.5 rounded-md">
                            <p class="text-white text-lg font-semibold">Ringkasan Peminjaman</p>
                        </div>
                        <div class="px-4 flex">
                            <div class="h-1/2 mt-4 flex">
                        ...
                        <div class="mx-4">
                            <table class="w-full text-center">
                                <tr>
                                    <th class="w-1/6 border-sky-600 border">Jumlah</th>
                                    <th class="w-1/6 border-sky-600 border">Kode Buku</th>
                                    <th class="w-4/6 border-sky-600 border">Judul Buku</th>
                                </tr>
                                <tr>
                                    <td class="border-sky-600 border"><?= $row['Jumlah']; ?></td>
                                    <td class="border-sky-600 border"><?= $row['Kode']; ?></td>
                                    <td class="border-sky-600 border"><?= $row['Judul']; ?></td>
                                </tr>
                            </table>
                        </div>
                        <form action="" method="post">
                            ...
                            <button type="submit" name="confirm" class="bg-sky-600 w-[1238px] py-1.5 text-white bottom-4 rounded-lg absolute hover:bg-sky-600 hover:text-white">Konfirmasi</button>
                        </form>
                    </div>
                    <div class="bg-white h-fit w-1/5 p-4 rounded-lg shadow-md">
                        <div class="h-96  ">
                            <img src="../assets/img/<?= $row['Gambar']; ?>" alt="" class="w-full h-96 rounded-lg">
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
